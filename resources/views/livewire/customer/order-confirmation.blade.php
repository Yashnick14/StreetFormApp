<div class="bg-gray-100 min-h-screen flex items-center justify-center p-6">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden w-full max-w-6xl">
        ...
        <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 rounded-xl p-4">
                    <h3 class="text-base font-medium text-slate-900 mb-4">Order Details</h3>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-slate-500 text-sm font-medium">Order Number</p>
                            <p class="text-slate-900 text-sm font-medium mt-2">{{ $order->_id }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-indigo-700 text-sm font-semibold mt-2">
                ...
                <!-- Shipping Info -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <h3 class="text-base font-medium text-slate-900 mb-4">Shipping Information</h3>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            <p class="text-slate-500 text-sm font-medium">Customer</p>
                            <p class="text-slate-900 text-sm font-medium mt-2">{{ $order->firstname }} {{ $order->lastname }}</p>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm font-medium">Phone</p>
                            <p class="text-slate-900 text-sm font-medium mt-2">{{ $order->phone }}</p>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <p class="text-slate-900 text-sm font-medium mt-2">{{ $order->email }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <!-- Items -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <h3 class="text-base font-medium text-slate-900 mb-4">Order Items ({{ count($items) }})</h3>
                    <div class="space-y-4">
                        @foreach($items as $item)
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    <h4 class="text-sm font-medium text-slate-900">{{ $product->name ?? 'Unknown' }}</h4>
                                    <p class="text-slate-500 text-xs mt-2">Size: {{ $item->ordersize ?? 'N/A' }}</p>
                                    <p class="text-slate-500 text-xs mt-1">Qty: {{ $item->orderquantity }}</p>
                                </div>
                ...
                <!-- Summary -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <h3 class="text-base font-medium text-slate-900 mb-4">Order Summary</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <p class="text-slate-500 text-sm">Subtotal</p>
                            <p class="text-slate-900 text-sm font-semibold">${{ number_format($order->totalprice, 2) }}</p>
                        </div>
                        <div class="flex justify-between">
                            <p class="text-slate-500 text-sm">Shipping</p>
        ...
        <!-- Footer -->
        <div class="bg-gray-100 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p class="text-slate-500 text-sm">
                Need help? <a href="javascript:void(0)" class="text-indigo-700 hover:underline">Contact us</a>
            </p>
            <a href="{{ url('/') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg">
                Continue Shopping
            </a>
        </div>
    </div>
