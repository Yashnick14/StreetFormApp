<div>
    ...
    <!-- Search + Top Buttons -->
    <div class="px-4 sm:px-6 lg:px-8 mt-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <!-- Search -->
        <div class="w-full sm:max-w-md relative">
            <input wire:model.live.debounce.500ms="search"
                   ...
                   class="block w-full rounded-md border-0 py-2 text-gray-900 shadow-sm
                          ...
                          focus:ring-2 focus:ring-inset focus:ring-indigo-600 text-sm pr-10">
                    ...
                    <table class="min-w-full divide-y divide-gray-300 text-sm">
                        <thead class="bg-gray-50 text-xs sm:text-sm">
                            <tr>
                                ...
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Order ID</th>
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Customer</th>
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Email</th>
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Total</th>
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Date</th>
                                <th class="px-3 sm:px-6 py-3 text-left font-semibold text-gray-700 uppercase">Status</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200 text-xs sm:text-sm">
                            @forelse ($orders as $order)
                                <tr class="hover:bg-gray-50 @if($selectedOrderId == $order->_id) bg-gray-50 @endif">
                                    ...
                                    <!-- Order Info -->
                                    <td class="px-3 sm:px-6 py-4 font-semibold">{{ $order->order_number ?? $order->_id }}</td>
                                    <td class="px-3 sm:px-6 py-4">{{ $order->firstname }} {{ $order->lastname }}</td>
                                    <td class="px-3 sm:px-6 py-4">{{ $order->email }}</td>
                                    <td class="px-3 sm:px-6 py-4">${{ number_format($order->totalprice, 2) }}</td>
                                    <td class="px-3 sm:px-6 py-4">{{ $order->orderdate->format('Y-m-d') }}</td>

                                    <!-- Status -->
                                    <td class="px-3 sm:px-6 py-4">
                                        <span class="inline-flex px-2 py-1 text-xs sm:text-sm font-semibold rounded-md
    ...
    <div x-data="{ open: @entangle('showStatusModal') }">
        <div x-show="open" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
